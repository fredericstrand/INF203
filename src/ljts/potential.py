...
class LJTS(Potential):
    ...
    def potential_energy(self, pos_i: np.ndarray, pos_j: np.ndarray, box_size: np.ndarray):
        """
        Calculate LJTS potential energy between two molecules.
        ...
        # Return zero if beyond cutoff
        if r2 >= self.cutoff**2:
            return 0.0
        
        # Calculate Lennard-Jones potential with shifting
        inv_r2 = 1.0 / r2
        inv_r6 = inv_r2**3
        inv_r12 = inv_r6**2
